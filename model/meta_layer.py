...
class GATConv_V2(MessagePassing):
    def __init__(
            self,
        ...
        # message function
        self.msg_func = torch.nn.Sequential(*[
            torch.nn.Linear(2 * self.out_channels + 8, self.out_channels),
            torch.nn.ReLU(),
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj, b=None,
                ...
                return_attention_weights=None, node_idx=None, node_idx_inv=None, edge_idx=None,
                edge_idx_inv=None):
        ...
        if b is not None:
            ff_in = torch.cat([b[edge_in], -b[edge_in]], dim=1).float()
            ff_out = torch.cat([-b[edge_out], b[edge_out]], dim=1).float()
        ...
        out = self.propagate(edge_index, x=x_x, alpha=alpha, size=size, state_prev=x, node_idx=node_idx
                             , node_idx_inv=node_idx_inv, b=b)
    ...
    def message(self, x_j: Tensor, x_i: Tensor, alpha: Tensor, b_i, b_j) -> Tensor:
#         x_j = self.msg_func(torch.cat([x_j, b_j, -1*b_j, x_i, b_i, -1*b_i], dim=1)) if b_i is not None else torch.cat([x_j, x_i], dim=1)
    ...
    def update(self, msg_agg, state_prev, node_idx, node_idx_inv):
        ...
        for i in range(len(node_idx)):
            ...
            if i == 0:
                ...
                aux = self.update_func3(msg, state)
            elif i == 3:
                aux = self.update_func4(msg, state)
            elif i == 4:
                aux = self.update_func5(msg, state)
...
class GATConv_meta(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # message function
        self.msg_func = torch.nn.Sequential(*[
            torch.nn.Linear(2 * self.out_channels, self.out_channels),
            torch.nn.ReLU(),
            torch.nn.Linear(self.out_channels, self.out_channels),
            torch.nn.ReLU(),
            torch.nn.Linear(self.out_channels, self.out_channels)
...
#         x_j = self.msg_func(torch.cat([x_j, x_i], dim=1))
        return alpha.unsqueeze(-1) * x_j
