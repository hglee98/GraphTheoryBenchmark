...
__all__ = ['MpnnGnn', 'MpnnGnn_non_meta']
...
class MpnnGnn(nn.Module):
    def __init__(self, config, test=False):
        ...
        assert self.aggregate_type in ['mean', 'add', 'min', 'max', 'sum', 'mul'], 'not implemented'

        # PROPAGATION LAYER
        if not self.include_b:
            print("Using Meta")
            self.GeoLayer1 = mpnn(2, self.hidden_dim, self.hidden_dim, self.num_module, self.aggregate_type)
            self.GeoLayer = mpnn(self.hidden_dim, self.hidden_dim, self.hidden_dim, self.num_module, self.aggregate_type)
        else:
        ...
        if self.node_only:
            ...
            self.graph_read_out = nn.Sequential(*[
                nn.Linear(graph_input_dim, divided_2_g),
                #                 nn.BatchNorm1d(divided_2_g),
                nn.ReLU(),
                nn.Linear(divided_2_g, divided_4_g),
                #                 nn.BatchNorm1d(divided_4_g),
                nn.ReLU(),
    ...
    def forward(self, x, edge_index=None, target_n=None, target_g=None, node_idx=None, node_idx_inv=None, batch=None):
        ...
        num_node = x.shape[0]
        if not self.include_b:
            state = self.GeoLayer1(x, edge_index, node_idx=node_idx, node_idx_inv=node_idx_inv)
...
class MpnnGnn_non_meta(nn.Module):
    def __init__(self, config, test=False):
        """ Multitask meta gnn """
        super(MpnnGnn_non_meta, self).__init__()
        self.config = config
        ...
        self.batch_size = config.test.batch_size if test else config.train.batch_size
        self.num_module = config.model.num_module
        assert self.aggregate_type in ['att', 'mean', 'add', 'min', 'max', 'sum', 'mul'], 'not implemented'

        # PROPAGATION LAYER
        if not self.include_b:
            print("Using Meta")
            self.GeoLayer1 = mpnn_non(2, self.hidden_dim, self.hidden_dim, self.aggregate_type)
            self.GeoLayer = mpnn_non(self.hidden_dim, self.hidden_dim, self.hidden_dim, self.aggregate_type)
        else:
            print("Include node attribute")
            self.GeoLayer = mpnn_non(self.hidden_dim, self.hidden_dim, self.hidden_dim, self.aggregate_type)
